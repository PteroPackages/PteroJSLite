        ...
        async connect() {
            ...
            this.ws.on('message', (m: MessageEvent) => {
                ...
                switch (packet.event) {
                    ...
                    case 'token expiring': this._heartbeat(); break;
                    case 'token expired': this.disconnect(); break;
                    case 'backup completed': this.onBackupComplete?.(JSON.parse(packet.args.join())); break;
                    case 'console output': this.onConsoleOutput?.(packet.args); break;
                    case 'daemon message': this.onDaemonOutput?.(packet.args); break;
                    case 'daemon error': this.onDaemonError?.(new Error(packet.args.join())); break;
                    case 'install started': this.onInstallStart?.(); break;
                    case 'install output': this.onInstallOutput?.(packet.args); break;
                    case 'install completed': this.onInstallComplete?.(); break;
                    case 'jwt error': this.onJWTError?.(new Error(packet.args.join())); break;
                    case 'stats': this.onStatsUpdate?.(conv.toCamelCase(JSON.parse(packet.args.join()))); break;
                    case 'status': this.onStatusUpdate?.(packet.args.join()); break;
                    case 'transfer logs': this.onTransferOutput?.(packet.args); break;
                    case 'transfer status': this.onTransferStatus?.(packet.args[0]); break;
                    default: this.onError?.(`received unknown event '${packet.event}'`); break;
                }
            });
